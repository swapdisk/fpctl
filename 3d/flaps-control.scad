...
// Face tabs
tab_thick=1.2;
ft=[[dmc_xy*2+dmc_ty+sm_rad-gp_high, 0, gp_high-dmc_xy*2-sm_rad*2-dmc_ty, tab_thick],
    [dmc_xy*2+dmc_ty+sm_rad-gp_high, gp_wide-tab_thick, gp_high-dmc_xy*2-sm_rad*2-dmc_ty, tab_thick],
    [dmc_xy-tab_thick, dmc_xy+sm_rad, tab_thick, gp_wide-sm_rad*2-dmc_xy*2],
    [dmc_xy-gp_high, dmc_xy+sm_rad, tab_thick, gp_wide-sm_rad*2-dmc_xy*2]];
...
translate([0, 0, -arch_rad*0.87+panel_thick+bezel_thick]) difference() {
    union() {
        // main control body
        ...
        // potentiometer mount
        rotate([90, 0, 0]) { 
            cylinder(total_wide, pot_reg_offset+pot_reg_wide-2, pot_reg_offset+pot_reg_wide-2);
            translate([-pot_reg_offset-pot_reg_wide+2, 0, 0])
                cube([(pot_reg_offset+pot_reg_wide-2)*2, 33, 33]);
        }
        // panel 
        translate([-arch_rad*panel_f/2,
            ...
            linear_extrude(bezel_thick+panel_thick) hull() {
                ...
                translate([arch_rad*panel_f-sm_rad, panel_l+total_wide+panel_r-sm_rad, 0]) circle(sm_rad);
    ...
    translate([arch_rad*panel_f/2,
                   -total_wide-panel_l, 
                   arch_rad*0.87])    
    translate([0, 0, -lab_thick]) linear_extrude(99) hull() {
        for (i = [0:3]) {
            c=lab_corners[i];
            translate([c[0]+c[2]+(i==1||i==2?-lab_mar:lab_mar),
                       c[1]+c[2]+(i==0||i==1?-lab_mar:lab_mar),
                       0]) circle(eps);
        }
    }
    ...
    rotate([90, -78, 0]) rotate_extrude(angle=24) translate([pot_reg_offset, 0, 0])
...
//        cylinder(999, arch_rad+detent_wide-26, arch_rad+detent_wide-26);
    ...
    // Panel only
    //translate([-49, -49, arch_rad*0.87-panel_thick-bezel_thick-99+eps]) cube([99, 99*2, 99]);
    // lower only
    //translate([-49, -49, arch_rad*0.87-panel_thick-bezel_thick]) cube([99, 99*2, 99]);
}
