...
translate([0, 0, -arch_rad*0.87+panel_thick+bezel_thick]) difference() {
    union() {
        ...
        // potentiometer mount
        rotate([90, 0, 0]) { 
            cylinder(total_wide, pot_reg_offset+pot_reg_wide-3, pot_reg_offset+pot_reg_wide-3);
            translate([-pot_reg_offset-pot_reg_wide+3, 0, 0])
                cube([(pot_reg_offset+pot_reg_wide-3)*2, 33, 33]);
        }
        // panel 
